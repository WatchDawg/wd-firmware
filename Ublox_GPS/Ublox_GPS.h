/*
  ...
  the RTCM messages to the user's selected HardwareSerial
...
//The following consts are used to configure the various ports and HardwareSerials for those ports. See -CFG-PRT.
...
public:
	...
	bool begin(HardwareSerial &serialPort); //Returns true if module is detected
	...
	bool sendCommand(ubxPacket outgoingUBX, uint16_t maxWait = 250); //Given a packet and payload, send everything including CRC bytes, return true if we got a response
...
//	void printPacket(ubxPacket *packet); //Useful for debugging
	...
	void setNMEAOutputPort(HardwareSerial &nmeaOutputPort);													  //Sets the internal variable for the port to direct NMEA characters to
	...
	bool waitForResponse(uint8_t requestedClass, uint8_t requestedID, uint16_t maxTime = 250); //Poll the module until and ack is received
	...
	bool getPVT(uint16_t maxWait = 1000);											 //Query module for latest group of datums and load global vars: lat, long, alt, speed, SIV, accuracies, etc. If autoPVT is disabled, performs an explicit poll and waits, if enabled does not block. Retruns true if new PVT is available.
	...
	bool getHPPOSLLH(uint16_t maxWait = 1000);										 //Query module for latest group of datums and load global vars: lat, long, alt, speed, SIV, accuracies, etc. If autoPVT is disabled, performs an explicit poll and waits, if enabled does not block. Retruns true if new PVT is available.
	...
	bool setUART1Output(uint8_t comSettings, uint16_t maxWait = 250); //Configure UART1 port to output UBX, NMEA, RTCM3 or a combination thereof
	bool setUART2Output(uint8_t comSettings, uint16_t maxWait = 250); //Configure UART2 port to output UBX, NMEA, RTCM3 or a combination thereof
	...
	bool enableRTCMmessage(uint8_t messageNumber, uint8_t portID, uint8_t secondsBetweenMessages, uint16_t maxWait = 250); //Given a message number turns on a message ID for output over given PortID
	...
	bool getProtocolVersion(uint16_t maxWait = 1000);	 //Queries module, loads low/high bytes
...
private:
	...
	//Variables
	HardwareSerial *_serialPort;			//The generic connection to user's chosen Serial hardware
	HardwareSerial *_nmeaOutputPort = 0;    //The user can assign an output port to print NMEA sentences if they wish
	HardwareSerial *_debugSerial;			//The HardwareSerial to send debug messages to if enabled
	...
	bool commandAck = false;			  //This goes true after we send a command and it's ack'd
