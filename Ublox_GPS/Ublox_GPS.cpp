...
bool SFE_UBLOX_GPS::begin(HardwareSerial &serialPort)
{
...
//void SFE_UBLOX_GPS::enableDebugging(HardwareSerial &debugPort)
//{
...
void SFE_UBLOX_GPS::setNMEAOutputPort(HardwareSerial &nmeaOutputPort)
{
...
//https://dspace.cvut.cz/bitHardwareSerial/handle/10467/65205/F3-BP-2016-Shkalikava-Anastasiya-Prenos%20polohove%20informace%20prostrednictvim%20datove%20site.pdf?sequence=-1
...
bool SFE_UBLOX_GPS::sendCommand(ubxPacket outgoingUBX, uint16_t maxWait)
{
...
bool SFE_UBLOX_GPS::isConnected()
{
...
bool SFE_UBLOX_GPS::waitForResponse(uint8_t requestedClass, uint8_t requestedID, uint16_t maxTime)
{
  ...
  while (xTaskGetTickCount() - startTime < pdMS_TO_TICKS(maxTime))
  {
...
bool SFE_UBLOX_GPS::enableRTCMmessage(uint8_t messageNumber, uint8_t portID, uint8_t secondsBetweenMessages, uint16_t maxWait)
{
  packetCfg.cls = UBX_CLASS_CFG;
...
bool SFE_UBLOX_GPS::setPortOutput(uint8_t portID, uint8_t outHardwareSerialSettings, uint16_t maxWait)
{
  ...
  //payloadCfg is now loaded with current bytes. Change only the ones we need to
  payloadCfg[14] = outHardwareSerialSettings; //OutProtocolMask LSB - Set outHardwareSerial bits
...
//Port 0=I2c, 1=UART1, 2=UART2, 3=USB, 4=SPI
//Bit:0 = UBX, :1=NMEA, :5=RTCM3
bool SFE_UBLOX_GPS::setPortInput(uint8_t portID, uint8_t inHardwareSerialSettings, uint16_t maxWait)
{
  //Get the current config values for this port ID
  ...
  //payloadCfg is now loaded with current bytes. Change only the ones we need to
  payloadCfg[12] = inHardwareSerialSettings; //InProtocolMask LSB - Set inHardwareSerial bits
...
bool SFE_UBLOX_GPS::getProtocolVersion(uint16_t maxWait)
{
  ...
  for (uint8_t extensionNumber = 0; extensionNumber < 10; extensionNumber++)
  {
    ...
    //Now we need to find "PROTVER=18.00" in the incoming byte HardwareSerial
